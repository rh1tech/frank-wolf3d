...
static void CAL_HuffExpand(byte *source, byte *dest, int32_t length, huffnode *hufftable)
{
    ...
    int32_t maxbits = length * 20;  // generous upper bound
#endif
    ...
    while(1)
    {
        ...
        if(--maxbits <= 0)
        {
            printf("HuffExpand: stuck! written=%d/%ld\n", written, (long)length);
            break;
        }
#endif
    }
...
#ifdef PICO_ON_DEVICE
        // Read compressed data into SRAM (not PSRAM) for fast Huffman decoding
        bufferseg = (int32_t *)malloc(compressed);
        if (!bufferseg) bufferseg = SafeMalloc(compressed);  // fallback to PSRAM
#else
...
#ifdef PICO_ON_DEVICE
        // Check if it's SRAM or PSRAM and free accordingly
        if ((uintptr_t)bufferseg < 0x11000000)
            free(bufferseg);
        else
            safe_free(bufferseg);
#else
